   ...
   $("#login_form").submit(function(e){
    ...
    $(".add_quiz").click(function(){
       $(".glyphicon-plus").toggle();
        ...
        $("#show-quiz-form").slideToggle();
        });
    function add_questions(quiz_id,i)
    {
        $("#quiz_list").hide();
        $("#add_questions").show();
    ...
    $("#quiz-form").submit(function(e){
           ...
           $.post("/quiz/add_quiz/",
               {
                   quiz_name : document.forms["quiz-form"]["quiz_name"].value,
                   user : document.forms["quiz-form"]["user"].value,
                   csrfmiddlewaretoken :document.forms["quiz-form"]["csrfmiddlewaretoken"].value
               },
               function(data,status)
               {
                   //alert(data);
                    $("#quiz_list").append(template(document.forms["quiz-form"]["user"].value,document.forms["quiz-form"]["quiz_name"].value));
                   $(".add_quiz").click();
                   document.forms["quiz-form"]["quiz_name"].value = "";
    ...
    $("#quiz_list").on('click','.quiz-panel',function(){
            $("#quiz_list").hide();
            quiz_id = $(this).find(".quiz_template").attr('id');
            $.get("/quiz/api/question/",function(data,status){
               ...
               {
                   if(data[i].quiz == quiz_id)
                   {
                       //console.log(data[i].quiz+' '+quiz_id);
                       data2.push(data[i]);
                   }
               }
                i=0;
